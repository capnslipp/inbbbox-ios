...
class ShotsProvider {
    ...
    func provideMyLikedShots() -> Promise<[ShotType]?> {
        if userStorageClass.userIsSignedIn {
    ...
    func provideShotsForUser(user: UserType) -> Promise<[ShotType]?> {
        assert(userStorageClass.userIsSignedIn, "Cannot provide shots for user when user is not signed in")
    ...
    func provideShotsForBucket(bucket: BucketType) -> Promise<[ShotType]?> {
        assert(userStorageClass.userIsSignedIn, "Cannot provide shots for bucket bucket when user is not signed in")
        return apiShotsProvider.provideShotsForBucket(bucket)
    }

    func nextPage() -> Promise<[ShotType]?> {
        assert(userStorageClass.userIsSignedIn, "Cannot provide shots for next page when user is not signed in")
        return apiShotsProvider.nextPage()
    }

    func previousPage() -> Promise<[ShotType]?> {
        assert(userStorageClass.userIsSignedIn, "Cannot provide shots for previous page when user is not signed in")
