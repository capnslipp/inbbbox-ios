...
protocol ShotImageType {
    ...
    ///
    /// - returns: URL to image with size 800x600. If `animated` attribute of the shot is `true`, this image will be animated.
    var hidpiURL: NSURL? { get }

    /// The normal image is typically 400x300, but may be smaller if created before October 4th, 2012.
    ///
    /// - returns: URL to image with size 400x300. If `animated` attribute of the shot is `true`, this image will be animated.
