    ...
    override func prepareLayout() {
        if let collectionView = collectionView {
            ...
            let calculatedItemWidth = round(CGRectGetWidth(collectionView.bounds)) -
                    fixedLeftMargin - fixedRightMargin
            let calculatedItemHeight = calculatedItemWidth * 3 / 4
            let calculatedVerticalSpacing = round(CGRectGetHeight(collectionView.bounds) / 2 -
                    calculatedItemHeight / 2)
            itemSize = CGSize(width: calculatedItemWidth, height: calculatedItemHeight)
            minimumLineSpacing = CGFloat(CGRectGetHeight(collectionView.bounds) -
